...
data class SpdxPackage(
    /**
     * A uniquely identifies this [SpdxPackage] within a SPDX document.
     */
    @JsonProperty("SPDXID")
    val spdxId: String,
    ...
    /**
     * The hompage URL. To represent a not present value [SpdxConstants.NONE] or [SpdxConstants.NOASSERTION] must be
     * used.
     */
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    val homepage: String,
    ...
    /**
     * The distribution source for the package. The value must be a single line text in one of the
     * following formats:
     *
     * 1. "Person: person name" or "Person: person name (email)"
     * 2. "Organization: organization name" or "Organization: organization name (email)"
    ...
    init {
        ...
        require(copyrightText.isNotBlank()) { "The copyrightText must not be blank." }

        require(downloadLocation.isNotBlank()) { "The downloadLocation must not be blank." }
